...
/**
 * [Admin Module] Cineplex App under Configure System Settings. Allow admin
 * users to create a new cineplex outlet, new cinemas (under a particular
 * outlet), view cinemas and delete existing cinemas.
 */
					...
					try {
						...
						if (files != null) {
							...
							selectedCineplex = (Cineplex) Serializer
									.deSerialize(path + "\\" + files[selected - 1].getName());

							System.out.println(
									"------- " + selectedCineplex.getVenue().toUpperCase() + ": ADD CINEMA -------\n");
							...
							do {
								...
								System.out.print(
										"Enter seat number followed by arrangement (0: Vacant, -1: Aisle, ,-2: Save): ");
							...
							try {
								...
								Serializer.serialize(root + "\\data\\cineplex\\" + files[selected - 1].getName(),
										selectedCineplex); // files[selected-1].getName() is the selected cineplex

								// Create new Cinema data file
								Serializer.serialize(root + "\\data\\cinema\\" + newCinema.getCinemaID() + ".dat",
										newCinema);
				...
				case 4:
					...
					try {
						...
						if (files != null) {
							...
							System.out.println("------- " + selectedCineplex.getVenue().toUpperCase()
									+ ": DELETE CINEMA -------\n");
							...
							for (int i = 0; i < cinemas.size(); i++) {
								...
								System.out.println(
										(i + 1) + ") " + curr.getCinemaID() + " (" + curr.getCinemaClass() + ")");
							}
							...
							try {
								...
								File fileToDelete = new File(
										root + "\\data\\cinema\\" + cinemaToDelete.getCinemaID() + ".dat");
