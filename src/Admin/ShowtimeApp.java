...
public class ShowtimeApp extends AppHelper {
	...
	public void runInterface() {
		...
		while (stay) {
			...
			try {
				...
				/*
				 * boolean check = false; while(!check) {
				 * System.out.println("Please enter a valid input"); input = sc.nextInt(); }
				 * 
				 */
			//	while (!sc.hasNextInt()) {
		...
		try {
			...
			while (!showtimeValid) {
				...
				try {
					...
					// System.out.println("The showtime added is : " +
					// newShowtime.getShowDateTime().toString());
			...
			try {
				...
				if (cineplexFiles != null) {
					for (int i = 0; i < cineplexFiles.length; i++) {
						Cineplex curr_cineplex = (Cineplex) Serializer
								.deSerialize(path_cineplex + "\\" + cineplexFiles[i].getName());
				...
				Cineplex selectedCineplex = (Cineplex) Serializer
						.deSerialize(path_cineplex + "\\" + cineplexFiles[cine_index - 1].getName());
				...
				try {
					...
					Serializer.serialize(root + "\\data\\movies\\" + selectedMovie.getMovieId() + ".dat",
							selectedMovie);
	...
	public void viewShowtime() {
		...
		try {
			...
			if (movieFiles != null) {
				for (int i = 0; i < movieFiles.length; i++) {
					Movie curr = (Movie) Serializer.deSerialize(path + "\\" + movieFiles[i].getName());
					System.out.println((i + 1) + ") " + curr.getTitle() + " [" + curr.getContentRating().toString()
							+ "] - " + curr.getShowingStatus());
				}
			}
			...
			try {
				...
				System.out.println("\nAvailable Showtimes at " + selectedCineplex.getVenue() + " for "
				...
				for (int i = 0; i < selectedMovie.getShowTimes().size(); i++) {
					...
					if (currShowTime.getCineplexID().equals(selectedCineplexId)) {
						...
						Cinema selectedCinema = (Cinema) Serializer
								.deSerialize(path_cinema + "\\" + currShowTime.getCinemaID() + ".dat");
						System.out.println("Cinema Hall " + currShowTime.getCinemaID() + " ("
								+ selectedCinema.getCinemaClass() + " Class)");
						System.out
								.println("Date and Time: " + dateTime + " (" + currShowTime.getShowTimeStatus() + ")");
	...
	/**
	 * Update details of an existing ShowTime listing such as the DateTime and
	 * Showing Status (i.e., Available, Sold_Out).
	 */
	//// (3) UPDATE LISTING
		...
		try {
			...
			if (showtimeList.size() > 0) {
				...
				for (int i = 0; i < showtimeList.size(); i++) {
					...
					Cineplex cineplex = (Cineplex) Serializer
							.deSerialize(root + "\\data\\cineplex\\" + currShowTime.getCineplexID() + ".dat");
